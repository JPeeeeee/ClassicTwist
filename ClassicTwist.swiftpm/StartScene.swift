...
//  WWDC23
//
//  Created by João Pedro Vieira Santos on 05/04/23.
//
import SpriteKit
...
class StartScreen: SKScene {
    ...
    private var drum_game_hint: SKSpriteNode!
    
    private var flag_in_scene: Int = 0
    ...
    private func setup_stage () {
        ...
        bateria = SKSpriteNode(imageNamed: "bateriaBroken")
    ...
    override func didMove(to view: SKView) {
        ...
        button.position = CGPoint(x: frame.midX, y: frame.midY - 250)
        button.zPosition = 5
        
        cortina = SKSpriteNode(texture: SKTexture(imageNamed: "cortina"), size: CGSize(width: frame.width, height: frame.height))
        cortina.position = CGPoint(x: frame.midX, y: frame.midY)
        cortina.zPosition = 4
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if button.contains(location) {
            let scale = SKAction.scale(by: 0.9, duration: 0.2)
            button.run(scale)
        }
    }
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if button.contains(location) && flag_button != 0 {
            button.run(animations(animation_type: "scaleButton", node: button))
            button.run(animations(animation_type: "fadeOut", node: button))
            cortina.run(animations(animation_type: "moveLeft", node: cortina))
            button.run(animations(animation_type: "buttonCircleOfLife", node: button))
            ...
            SoundManager.backgroundManager.playLoop(sound: .backgroundMusic)
            
        } else if bateria != nil && bateria.contains(location) && flag_in_scene != 0 {
            let transition_close_curtain = SKTransition.doorsCloseHorizontal(withDuration: 1.5)
            let newScene = CurtainTransitionScene()
            newScene.set(newScene: DrumGameScene())
        ...
        } else if text_box != nil && text_box.contains(location) && flag_in_scene != 0 {
            ...
            if text_model.title != "End" {
                ...
                drum_game_hint = SKSpriteNode(imageNamed: "DrumGameHint")
                drum_game_hint.size = CGSize(width: drum_game_hint.size.width, height: drum_game_hint.size.height)
                drum_game_hint.position = CGPoint(x: frame.midX, y: frame.height * 0.9)
                drum_game_hint.zPosition = 4
                drum_game_hint.alpha = 0
                
                addChild(drum_game_hint)
                
                drum_game_hint.run(animations(animation_type: "fadeIn", node: drum_game_hint))
                drum_game_hint.run(animations(animation_type: "textBoxBounce", node: drum_game_hint))
            }
        }
    }
}
